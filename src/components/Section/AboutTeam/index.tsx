...
import { SkeletonTeamItem } from "@/components/Skeleton";
import { getTeamsSocmed } from "@/services/teams";
import {
  TEAM_SOCMED_LINKEDLN,
  TEAM_SOCMED_FACEBOOK,
  TEAM_SOCMED_X,
  TEAM_SOCMED_INSTAGRAM,
  TEAM_SOCMED_YOUTUBE,
  TEAM_SOCMED_TIKTOK,
} from "@/store/constants";
import he from "he";

type TAboutTeamProps = {
  list: TTeams[] | null;
};

const SocmedIcon = (name: string) => {
  if (name == TEAM_SOCMED_LINKEDLN) {
    return (
  ...
  if (name == TEAM_SOCMED_FACEBOOK) {
    return (
      <svg width="25" height="25" viewBox="0 0 25 25" fill="none">
        <circle cx="12.5" cy="12.5" r="12.5" className="fill-black" />
        <path
          d="M5.95215 7.9955C5.95215 7.52452 6.34352 7.14282 6.8264 7.14282H16.9827C17.4655 7.14282 17.8569 7.52452 17.8569 7.9955V18.1949C17.8569 18.6659 17.4655 19.0476 16.9827 19.0476H6.8264C6.34352 19.0476 5.95215 18.6659 5.95215 18.1949V7.9955ZM9.62998 17.1086V11.7329H7.84352V17.1086H9.62998ZM8.73712 10.9985C9.35989 10.9985 9.74754 10.5863 9.74754 10.0699C9.73637 9.54238 9.36063 9.14133 8.74902 9.14133C8.13742 9.14133 7.73786 9.54312 7.73786 10.0699C7.73786 10.5863 8.12551 10.9985 8.72521 10.9985H8.73712ZM12.3889 17.1086V14.1064C12.3889 13.9457 12.4008 13.7849 12.4484 13.6704C12.5771 13.3497 12.871 13.0171 13.3651 13.0171C14.0117 13.0171 14.2699 13.5096 14.2699 14.2329V17.1086H16.0563V14.0253C16.0563 12.3735 15.1754 11.6056 13.9998 11.6056C13.0519 11.6056 12.627 12.1265 12.3889 12.4933V12.5119H12.377L12.3889 12.4933V11.7329H10.6032C10.6255 12.2373 10.6032 17.1086 10.6032 17.1086H12.3889Z"
          className="fill-white"
        />
  ...
  if (name == TEAM_SOCMED_X) {
    return (
      <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
        <circle cx="13.0239" cy="12.5" r="12.5" className="fill-black" />
        <path
  ...
  if (name == TEAM_SOCMED_INSTAGRAM) {
    return (
      <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
        <circle cx="12.7856" cy="12.5" r="12.5" className="fill-black" />
        <path
  ...
  if (name == TEAM_SOCMED_YOUTUBE) {
    return (
      <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
        <circle cx="12.5479" cy="12.5" r="12.5" className="fill-black" />
        <path
  ...
  if (name == TEAM_SOCMED_TIKTOK) {
    return (
      <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
        <circle cx="13.3096" cy="12.5" r="12.5" className="fill-black" />
        <path
...
const AboutTeam = (props: TAboutTeamProps) => {
  const { list } = props;
  const t = useTranslations("About");
  const [openModal, setOpenModal] = useState(false);
  const [selectedTeam, setSelectedTeam] = useState<TTeams | null>(null);
  const [selectedPhoto, setSelectedPhoto] = useState<string | null>(null);
  const [selectedDesc, setSelectedDesc] = useState<string>("");
  const [selectedSocmed, setSelectedSocmed] = useState<TTeamsSocmed[] | null>(
    null
  );
  const localActive = useLocale();

  const handleOpenModal = async (data: TTeams) => {
    const teamDetail = await getTeamsSocmed(data.id);

    setSelectedTeam(data);
    setSelectedSocmed(teamDetail);
    setOpenModal(true);
  };
  ...
  useEffect(() => {
    if (selectedTeam !== null) {
      const photoData = imagePath(selectedTeam.id, selectedTeam.photo, "teams");
      const descData =
        localActive == "en"
          ? selectedTeam?.bio_description_en
          : selectedTeam?.bio_description_id;

      setSelectedPhoto(photoData);
      setSelectedDesc(descData);
    }
  }, [selectedTeam, localActive]);
  ...
  return (
    <>
      ...
      <Modal isOpen={openModal} className="z-[300]" noClickBg>
        <div className="w-full h-screen md:h-auto overflow-auto max-w-[901px] mx-auto relative bg-white py-32 px-16 md:py-[3.125rem] md:px-[2.5rem] modal-confirm-content">
          <div
            className="absolute right-[1rem] md:right-[1.25rem] top-[1rem] md:top-[1.25rem] cursor-pointer group"
          ...
          <div className="flex flex-col md:flex-row gap-32 md:gap-[100px] min-h-[400px]">
            <div className="w-full md:w-1/2">
              <div className="flex items-center">
                <div className="w-[86px] aspect-square relative shrink-0">
                  {selectedTeam !== null && selectedPhoto !== null && (
                    <Image
                      src={selectedPhoto}
                      alt="Team 1"
                      fill={true}
