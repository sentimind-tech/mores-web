...
export const MoresightSubNav = () => {
  const t = useTranslations("MoresightPage.subNav");
  const [activeMenuId, setActiveMenuId] = useState<string | null>(null);

  const scrollToView = (id: string) => {
    setActiveMenuId(id);
    const targetId = id === "overview" ? "moresight-overview" : id;
  ...
  return (
    // 💡 PENYESUAIAN 1: Menghapus h-[80px] agar tingginya dinamis (py-4) saat bertumpuk di mobile, h-md:[80px] tetap aman di desktop
    <div className="w-full bg-white border-b border-[#989898] sticky top-[70px] z-[99] py-4 md:h-[80px] md:py-0 flex items-center">
      
      {/* 💡 PENYESUAIAN 2: Mengubah flex-col di mobile agar teks bertumpuk ke bawah, dan kembali ke flex-row md:items-center di desktop */}
      <div className="max-w-[1280px] mx-auto section-padding-x md:px-8 lg:px-12 xl:px-16 w-full flex flex-col md:flex-row items-start md:items-center justify-start overflow-hidden gap-y-2 md:gap-y-0">
        ...
        {/* 💡 PENYESUAIAN 3: Menghapus 'hidden' agar tulisan MORESIGHT muncul di mobile, menambahkan mb-1 block md:mb-0 */}
        <div className="block font-supplymono text-[16px] font-normal text-black uppercase min-w-[223px] tracking-wide mb-1 md:mb-0">
          {t("title")}
        </div>

        {/* 💡 PENYESUAIAN 4: Garis pembatas vertikal tetap disembunyikan di mobile (hidden md:block) karena posisi menu sudah pindah ke bawah teks */}
        <div className="hidden md:block h-[35px] w-[1px] bg-[#A0A0A0] mx-6 lg:mx-8" />

        {/* SISI KANAN / BAWAH: Menu Navigasi */}
