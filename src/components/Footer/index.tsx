...
import CardFooterAddress from "../Cards/CardFooterAddress";
import FooterNavlink from "./NavLink";

const Footer = async () => {
  const linkedin = await getConfigByKey(CONFIG_CONTACT_US_LINKEDIN);
  const instagram = await getConfigByKey(CONFIG_CONTACT_US_INSTAGRAM);
  const youtube = await getConfigByKey(CONFIG_CONTACT_US_YOUTUBE);
  const socmedX = await getConfigByKey(CONFIG_CONTACT_US_X);
        ...
        <div className="flex justify-between gap-20 mb-32">
          <div className="flex items-center gap-24">
            ...
            <div className="flex gap-12">
              {linkedin && (
                <Link href={linkedin.value.url} target="_blank">
              ...
              {instagram && (
                <Link href={instagram.value.url} target="_blank">
              ...
              {youtube && (
                <Link href={youtube.value.url} target="_blank">
                  <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
                    <ellipse
                      cx="13.3096"
                      cy="12.5"
              ...
              {socmedX && (
                <Link href={socmedX.value.url} target="_blank">
                  <svg width="26" height="25" viewBox="0 0 26 25" fill="none">
                    <ellipse
                      cx="13.2144"
                      cy="12.5"
            ...
            <div className="block">
              ...
              {phoneConfig && (
                ...
                >
                  ...
                  >
                    {phoneConfig.value.title?.toLowerCase()}
                  </BodyText>
                </Link>
              )}
            </div>
            <div className="block">
              {oficeConfig && (
                <div className="max-w-[212px] mb-[2.875rem]">
                  <CardFooterAddress
                    title={oficeConfig.value.title}
                    address={oficeConfig.value.address}
                  />
                </div>
              )}
              {workshopConfig && (
                <div className="max-w-[212px]">
                  <CardFooterAddress
                    title={workshopConfig.value.title}
                    address={workshopConfig.value.address}
                  />
                </div>
              )}
            </div>
            ...
            <div className="block">
              {emailConfig && (
                ...
                >
                  ...
                  <BodyText
                    type="body2"
                    className="uppercase !text-12 leading-[0.75rem] font-supplymono"
                  >
